...
// }
//
//         [[nodiscard]] inner_type operator<(const inner_type &a, const inner_type &b) { subst(less, a, b) }
//         [[nodiscard]] inner_type operator>(const inner_type &a, const inner_type &b) { subst(more, a, b) }
//         [[nodiscard]] inner_type operator<=(const inner_type &a, const inner_type &b) { subst(less_equal, a, b) }
//         [[nodiscard]] inner_type operator>=(const inner_type &a, const inner_type &b) { subst(more_equal, a, b) }
//         [[nodiscard]] inner_type operator==(const inner_type &a, const inner_type &b) { subst(equal, a, b) }
//         [[nodiscard]] inner_type operator!=(const inner_type &a, const inner_type &b) { subst(not_equal, a, b) }
//         [[nodiscard]] inner_type operator+(const inner_type &a, const inner_type &b) { subst_digit_only(add, a, b) }
//         [[nodiscard]] inner_type operator-(const inner_type &a, const inner_type &b) { subst_digit_only(subtract, a, b) }
//         [[nodiscard]] inner_type operator*(const inner_type &a, const inner_type &b) { subst_digit_only(multiply, a, b) }
//         [[nodiscard]] inner_type operator/(const inner_type &a, const inner_type &b) { subst_digit_only(divide, a, b) }
//         [[nodiscard]] inner_type operator%(const inner_type &a, const inner_type &b) { return mod(a, b); }
//
...
//
//         inner_type & operator+=(inner_type &a, const inner_type &b) { checkType(a, b, "+="); a = (a + b); return a; }
//         inner_type & operator-=(inner_type &a, const inner_type &b) { checkType(a, b, "-="); a = (a - b); return a; }
//         inner_type & operator*=(inner_type &a, const inner_type &b) { checkType(a, b, "*="); a = (a * b); return a; }
//         inner_type & operator/=(inner_type &a, const inner_type &b) { checkType(a, b, "/="); a = (a / b); return a; }
//         inner_type & operator%=(inner_type &a, const inner_type &b) { checkType(a, b, "%="); a = (a % b); return a; }
...
#define subst(fun, a, b) \
    switch (get_type(a)) { \
        case types::INT: return fun<typeInt>(a, b); \
        case FLOAT: return fun<typeFloat>(a, b); \
        case STRING: return fun<std::string>(a, b); \
        default: throw std::runtime_error("unknown type"); \
    }
...
#define subst_digit_only(fun, a, b)  \
    ...
    [[nodiscard]] const inner_type& operator<(const variable &a, const variable &b) const { subst(less, a.second, b.second) }
    [[nodiscard]] const inner_type& operator>(const inner_type &a, const inner_type &b) const { subst(more, a, b) }
    [[nodiscard]] const inner_type& operator<=(const inner_type &a, const inner_type &b) const { subst(less_equal, a, b) }
    [[nodiscard]] const inner_type& operator>=(const inner_type &a, const inner_type &b) const { subst(more_equal, a, b) }
    [[nodiscard]] const inner_type& operator==(const variable &a, const variable &b) const { subst(equal, a, b) }
    [[nodiscard]] const inner_type& operator!=(const inner_type &a, const inner_type &b) const { subst(not_equal, a, b) }
    [[nodiscard]] const inner_type& operator+(const variable &a, const variable &b) const { subst_digit_only(add, a.second, b.second) }
    [[nodiscard]] const inner_type& operator-(const inner_type &a, const inner_type &b) const { subst_digit_only(subtract, a, b) }
    [[nodiscard]] const inner_type& operator*(const variable &a, const variable &b) const { subst_digit_only(multiply, a, b) }
    [[nodiscard]] const inner_type& operator/(const inner_type &a, const inner_type &b) const { subst_digit_only(divide, a, b) }
    [[nodiscard]] const inner_type& operator%(const inner_type &a, const inner_type &b) const { return mod(a, b); }
...
protected:
    ...
    [[nodiscard]] static short get_type(const variable &d) {
        if (can_cast<interval::interval<typeInt>>(d.second)) return INT;
        if (can_cast<interval::interval<typeFloat>>(d.second)) return FLOAT;
        if (can_cast<interval::interval<std::string>>(d.second)) return STRING;
        throw std::runtime_error("Function: can_cast(), error: UNKNOWN TYPE");
    }

    static void checkType(const variable &a, const variable &b, const std::string &name_op) {
        if (get_type(b) != get_type(a))
            throw std::logic_error("Function operator " + name_op + ", error: could not compare different types!");
    }

    template<typename T>
    [[nodiscard]] static const inner_type& less_in(
            const std::function<void(interval::interval<T> &it, const inner_type &a)> &it,
            const variable &a,
            const variable &b,
            const std::string &n) {
    ...
    template<typename T>
    [[nodiscard]] static const inner_type& more(const inner_type &a, const inner_type &b) {
        return less_in<T>([](interval::interval<T> &buf, const inner_type &a) {
            buf.add_interval(std::get<interval::interval<T>>(a).any().value(),interval::maximal<T>());
        }, a, b, ">");
    }
    ...
    template<typename T>
    [[nodiscard]] static const inner_type& less_equal(const inner_type &a, const inner_type &b) {
        return less_in<T>([](interval::interval<T> &buf, const inner_type &a) {
            buf.add_interval(interval::minimal<T>(), std::get<interval::interval<T>>(a).any().value());
            buf.add_point(std::get<interval::interval<T>>(a).any().value());
        }, a, b, "<=");
    ...
    template<typename T>
    [[nodiscard]] static const inner_type& more_equal(const inner_type &a, const inner_type &b) {
        return less_in<T>([](interval::interval<T> &buf, const inner_type &a) {
            buf.add_interval(std::get<interval::interval<T>>(a).any().value(), interval::maximal<T>());
            buf.add_point(std::get<interval::interval<T>>(a).any().value());
        }, a, b, ">=");
    ...
    template<typename T>
    [[nodiscard]] static inner_type equal(const variable &a, const variable &b) {
        return less_in<T>([](interval::interval<T> &buf, const inner_type &a, const inner_type &b) {
            buf.add_point(std::get<interval::interval<T>>(a).any().value());
        }, a.second, b.second, "==");
    }

    template<typename T>
    [[nodiscard]] static inner_type not_equal(const variable &a, const variable &b) {
        return less_in<T>([](interval::interval<T> &buf, const inner_type &a, const inner_type &b) {
            buf.add_interval(std::get<interval::interval<T>>(a).any().value(), interval::maximal<T>());
            buf.add_interval(interval::minimal<T>(), std::get<interval::interval<T>>(b).any().value());
        }, a.second, b.second, "!=");
    }

    template<typename T>
    [[nodiscard]] static const inner_type& add(const inner_type &a, const inner_type &b) {
        checkType(a, b, "+");
        return std::get<interval::interval<T>>(a)
               + std::get<interval::interval<T>>(b).any().value();
    }
    ...
    template<typename type>
    [[nodiscard]] static inner_type multiply(const variable &a, const variable &b) {
        checkType(a, b, "*");
        return std::get<interval::interval<type>>(a.second)
               * std::get<interval::interval<type>>(b.second).any().value();
    }
    ...
    [[nodiscard]] inner_type merge(
                const variable &a, const variable &b, const std::size_t op_number) {
        switch (op_number) {
            case 0: return this->operator*(a, b);
            case 2: return this->operator==(a, b);
            case 3: return this->operator!=(a, b);
            case 4: return this->operator>(a, b);
            case 5: return this->operator<(a, b);
            case 6: return this->operator>=(a, b);
