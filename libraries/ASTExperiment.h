                        ...
                        for (auto &i : ld) {
                            for (auto &j : rd) {
                                ...
                                if (i.front()->name.empty()) {
                                    *out.front() = *j.front();
                                    check(out.front()->data, i.front()->data);
                                    check(i.front()->data, out.front()->data);
                                    if (save) {
                                        out.front()->history = {op, i.front()};
                                        out.front()->parent = j.front();
                                    }
                                    apply_operator(out.front()->data, i.front()->data, op);
                                }
                                else {
                                    *out.front() = *i.front();
                                    check(out.front()->data, j.front()->data);
                                    check(j.front()->data, out.front()->data);
                                    if (save) {
                                        out.front()->history = {op, j.front()};
                                        out.front()->parent = i.front();
                                    }
                                    apply_operator(out.front()->data, j.front()->data, op);
                                }
                                buf_out.insert(out);
                            }
                        }
